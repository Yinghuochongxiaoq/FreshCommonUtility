    ...
    public static class DeepCopyHelper
    {
        ...
        static Func<object, Dictionary<Identity, object>, object> CreateCloneMethod1(Type type, Dictionary<Identity, object> objects)
        {
            ...
            if (!type.IsArray)
            {
                ...
                il.Emit(OpCodes.Call, typeof(Identity).GetConstructor(new[] { typeof(int), typeof(RuntimeTypeHandle) }));
                ...
                {
                    if (!field.FieldType.GetTypeInfo().IsValueType && field.FieldType != typeof(String))
                    {
                        //不符合条件的字段，直接忽略，避免报错。  
                        if ((field.FieldType.IsArray && (field.FieldType.GetArrayRank() > 1 || (!(tmptype = field.FieldType.GetElementType()).GetTypeInfo().IsValueType && tmptype != typeof(String) && tmptype.GetConstructor(Type.EmptyTypes) == null))) ||
                            (!field.FieldType.IsArray && field.FieldType.GetConstructor(Type.EmptyTypes) == null))
                            break;
                        il.Emit(OpCodes.Ldloc_1);
                ...
                for (type = type.GetTypeInfo().BaseType; type != null && type != typeof(object); type = type.GetTypeInfo().BaseType)
                {
        ...
        public static object DeepCopyRecursion(object srcobj)
        {
            ...
            if (srcObjType.GetTypeInfo().IsValueType || srcObjType.GetTypeInfo()==typeof(string).GetTypeInfo())
            {
                return srcobj;
            }
            // Is array  
            ...
            // is List or map  
            else if (srcObjType.GetTypeInfo().IsGenericType)
            {
